...
def prepare(worktree, branch, pane):
...
def send_report(config_path, status, message, timeout):
    ...
    try:
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as client:
            ...
            # Drain until the server closes, as its native client does. This
            # detects connection errors but is not a delivery acknowledgement.
...
def main(argv=None):
    ...
    try:
        if args.command == "prepare":
            print(prepare(args.worktree, args.branch, args.pane))
        else:
