...
# `.herdr/workstreams.sh` in the target repo, sourced below. See README.md for
# the contract.
#
# Usage: [HERDR_WS_DESC=<phrase>] [HERDR_WS_MODEL=<model>] [HERDR_WS_CONFIG_DIR=<dir>] \
#          bootstrap.sh <slug> [task...]
#
# HERDR_WS_DESC names the dev pane. A short phrase saying what is being built, in the
# user's terms rather than the branch's. Unset, it falls back to the slug.
#
# HERDR_WS_MODEL pins the stream to a model. Unset, it takes the project's
# HERDR_WS_DEFAULT_MODEL so every stream runs on the same model whoever started it.
#
# HERDR_WS_CONFIG_DIR picks the Claude profile the stream runs under. Unset, it takes
# the project's HERDR_WS_DEFAULT_CONFIG_DIR.
#
...
: "${HERDR_WS_DEFAULT_CONFIG_DIR:=$HOME/.claude-field}"
...
# implementer starts the artifact, and sets up any state isolation the project
# needs, per its project's brief; bootstrap only opens the pane at the worktree.
second=""
